...
const register = async (req, res) => {
  try {
    ...
    try {
      ...
      if (result && result.insertId) userId = result.insertId;
    } catch (dbErr) {
      console.warn('Database query bypassed:', dbErr.message);
    }

    // Dispatch real registration confirmation email to ANY specified user email
    await sendRegistrationEmail(email, name, roll_no, department);
...
const login = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      ...
      process.env.JWT_SECRET || 'gdg_freshers_secret_key_2026',
      { expiresIn: '24h' }
    );
